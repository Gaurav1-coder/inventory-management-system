...
const getOrder = async (req, res) => {
    try {
        const orders = await Order.find({})
            .populate("Product.product", "product price") 
        ...
        const formattedOrders = orders.map(order => {
            ...
            return {
                ...orderObj,
                customer: orderObj.user, // Frontend expects customer
                total: orderObj.totalAmount, // Frontend expects total
            };
        });
...
const updatestatusOrder = async (req, res) => {
    try {
        ...
        if (updates.Product && Array.isArray(updates.Product)) {
            updates.totalAmount = updates.Product.reduce((sum, item) => sum + item.quantity * (item.price || 0), 0);
        }
