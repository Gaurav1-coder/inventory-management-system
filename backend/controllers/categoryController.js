...
const createCategory = async (req, res) => {
  try {
    ...
    await ActivityLog.create({
      ...
      user: req.user._id,
      details: `Category ${category} was created.`
    });
  ...
  try {
    ...
    const enrichedCategories = await Promise.all(
      categories.map(async (cat) => {
        const count = await Product.countDocuments({ category: cat.category });
        return {
          ...cat.toObject(),
          productCount: count
        };
...
const deleteCategory = async (req, res) => {
  try {
    const id = req.params.id;
    if (!id) return res.status(404).json("category was not found.");
